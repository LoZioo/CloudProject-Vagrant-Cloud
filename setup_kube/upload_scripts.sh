...
echo -e "${RED}Upload degli script sugli host $BOLDRED$NODES$RED con$NC"
if [[ $XHOSTIPS != "" ]] ; then
echo -e "${RED}IP esterni $BOLDRED$XHOSTIPS$RED e "
fi
echo -e "${RED}IP nel cluster k8s $BOLDRED$KHOSTIPS$NC"
...
echo -ne "${RED}gli script si trovano in $NC"
echo -e "${BOLDRED}/home/$KUSER/$(basename $PWD)$NC$RED"
echo -e "(si presume che su ogni host vi sia un utente ${BOLDRED}$KUSER$RED (\$KUSER in config.sh) e che "
