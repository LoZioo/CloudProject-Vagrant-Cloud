...
get_aws_net() {
   ...
   echo -e   "${RED}You can call this script as \". ${BASH_SOURCE[0]}\" to reload/list AWS related functions$NC"
}
...
read_nodes() {
   ...
   if [[ ${#XHOSTS[@]} -ne ${#KHOSTS[@]} ]] ; then
      echo -e "${RED}not all instances running (yet?)$NC"
      return
   fi
         ...
         --query Instances[0].InstanceId  || \
      {
         ...
         [[ $DESCRINSTOUT != None ]] && \
            echo -e "$DESCRINSTOUT running with name $N, private IP $IP_N" || \
            {
               ...
               echo -e  "${RED}... perhaps you should terminate instances and/or just retry ${NC}${IT}create_nodes${NC}${RED} as soon as$NC"
...
echo -ne "\n${RED}to re-read or create AWS networking environment:$NC "
echo -e "${IT}get_aws_net$NC"
