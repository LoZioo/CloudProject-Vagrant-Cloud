...
_get_key_pair() {
   KEYPAIRID=$(
      aws ec2 describe-key-pairs --filters Name=key-name,Values=$KEYNAME \
         --query KeyPairs[0].KeyPairId --output text
   ) && echo -e "found ssh key pair $KEYPAIRID, set KEYPAIRID to $KEYPAIRID" ||
      {
         echo "Failed to determine key pair Id KEYPAIRID"
         { [[ $0 == -bash ]] && return || exit; }
      }
   ...
   if [[ "x$KEYPAIRID" == "x" ]] || [[ $KEYPAIRID == None ]]; then
      ...
      ) && echo "created key pair $KEYPAIRID, set KEYPAIRID to $KEYPAIRID" || {
...
_get_sg() {
   SGID_OUT=$(aws ec2 describe-security-groups --filters \
      ...
      --output text) ||
      {
         echo "Failed to determine security group "
         { [[ $0 == -bash ]] && return || exit; }
      }
   ...
   if [[ "x$SGID" == "x" ]] || [[ $SGID == None ]]; then
      ...
      aws ec2 authorize-security-group-ingress --group-id $SGID \
         --protocol tcp --port 6443 --cidr $MYPUBIP/32 \
         --tag-specifications 'ResourceType=security-group-rule,Tags=[{Key=Name,Value=kcluster-client-6443}]' >/dev/null &&
         echo -e "        opened sg $SGID to $MYPUBIP", port 6443 ||
         echo "Failed to open security group $SGID, port 6443 to $MYPUBIP"
      ...
      aws ec2 authorize-security-group-ingress --group-id $SGID \
         --protocol tcp --port 6443 --cidr $EC2_CONNECT_CIDR \
         --tag-specifications 'ResourceType=security-group-rule,Tags=[{Key=Name,Value=ec2-connect-6443}]' >/dev/null &&
         echo -e "        opened sg $SGID to $EC2_CONNECT_CIDR", port 6443 ||
         echo "Failed to open security group $SGID, port 6443 to $EC2_CONNECT_CIDR"
      ...
      aws ec2 authorize-security-group-ingress --group-id $SGID \
         --ip-permissions IpProtocol=icmp,FromPort=-1,ToPort=-1,IpRanges=[{CidrIp=$MYPUBIP/32},{CidrIp=$KHOSTS_NETWORK}] >/dev/null &&
   ...
   echo "$SGID_OUT" | grep -q -E -e "^IPRANGES[[:blank:]]$MYPUBIP" ||
      echo -e "${BOLDRED}Warning:$NC Security group is not open to your IP $RED$MYPUBIP.${NC}\n\
         A possible reason is you created the s.g. when your cjient was on a different network.\n\
         ${BOLDRED}Solution${NC}: delete this security group and run function ${IT}get_aws_net${NC} again$NC"
...
_get_subnet() {
   ...
   ) && echo -e "found subnet $SUBNETID, set SUBNETID to $SUBNETID" || {
      echo "Failed to determine subnet"
      { [[ $0 == -bash ]] && return || exit; }
   }
...
_get_igw() {
   ...
   IGID=$(
      ...
      {
         echo "Failed to determine Internet gateway Id IGID"
         { [[ $0 == -bash ]] && return || exit; }
      }
   ...
   if [[ "x$IGID" == "x" ]] || [[ $IGID == None ]]; then
      ...
      ) && echo "created Internet gateway $IGID, set IGID to $IGID" || {
      ...
      aws ec2 attach-internet-gateway --internet-gateway-id $IGID --vpc-id $VPCID &&
         ...
         {
            echo "Failed to attach Internet gateway $IGID to $VPCID"
            { [[ $0 == -bash ]] && return || exit; }
         }
   fi
}
...
_get_rt() {
   ...
   if [[ "x$RTID" == "x" ]] || [[ $RTID == None ]]; then
      ...
      aws ec2 create-route --route-table-id $RTID --destination-cidr-block 0.0.0.0/0 --gateway-id $IGID >/dev/null &&
         ...
         {
            echo failed to create route to gateway $IGID for table $RTID
            { [[ $0 == -bash ]] && return || exit; }
         }
...
get_aws_net() {
   ...
   if [[ ! -f ../$KEYFILE ]]; then
      echo -e "${BOLDRED}file della chiave privata${BIT} $KEYFILE $NC${BOLDRED}non presente in$BIT $K8SDIR${NC}" >/dev/stderr
      echo -e "${BOLDRED}check${BIT} KEYFILE $NC${BOLDRED}in$BIT ../config.sh ${NC}($BIT$K8SDIR/config.sh${NC})" >/dev/stderr
      echo -e "${BOLDRED}and re-run this script:${BIT} . ./${BASH_SOURCE[0]}$NC" >/dev/stderr
      { [[ $0 == -bash ]] && return || exit; }
   else
      KEYNAME=$(basename $KEYFILE .pem)
      echo -e "chiave AWS \"$KEYNAME\""
   ...
   echo -e   "${RED}You can call this script as ${BIT}. ./${BASH_SOURCE[0]}$NC$RED to reload/list AWS related functions$NC"
}

echo -e "\n${RED}This script helps to manage (and create) an AWS cluster and prepare it to run k8s\n${NC}"
echo -e "sourcing setvars.sh and config.sh\n"
cd ..
...
if [[ x$VPCID == x ]] || [[ x$SUBNETID == x ]] || [[ xIGID == x ]] || [[ xIGID == x ]] || [[ xRTID == x ]] || [[ xSGID == x ]]; then
   echo -ne "\n${BOLDRED}Some variable above undefined$NC"
   echo -ne "\n${RED}to re-read or create AWS networking resources:$NC "
   echo -e "${IT}get_aws_net$NC"
   return
fi
         ...
         --query Instances[0].InstanceId  || \
      {
         ...
         [[ $DESCRINSTOUT != None ]] && \
            echo -e "$DESCRINSTOUT running with name $N, private IP $IP_N" || \
            {
               ...
               echo -e  "${IT}find_nodes${NC}${RED} prints out no, or terminating-only, instances$NC"
               return ;
            }
      }
   done
